...
TEST(CovarianceMatrixFactored, ctor_from_full_matrix) // NOLINT
{
  ...
  // call UUT
  const auto cov = tracking::math::CovarianceMatrixFactored<float32, 3>::FromList({
    {45, 52, 12},
    {52, 66, 16},
    {12, 16,  4}});
  // clang-format on
...
{
  // clang-format off
  tracking::math::CovarianceMatrixFactored<float32, 3> _cov = tracking::math::CovarianceMatrixFactored<float32, 3>::FromList({
    {1, 2, 3}, 
    {0, 1, 4}, 
    {0, 0, 1}}, {1, 2, 4});

  tracking::math::CovarianceMatrixFull<float32, 3> _covFull{_cov()};
  // clang-format on
};
