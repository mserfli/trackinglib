...
{
  // clang-format off
  auto U = conversions::TriangularFromList<float64, 4, false, true>({
  ...
  const auto expD = conversions::DiagonalFromList<float64, 4>({
    1.753069000330330,  0.277520518292310,  1.301391550356693,  1.094096507971646
  });
  // clang-format on

  // call UUT
  Rank1Update<float64, 4, true>::run(U, D, sigma, w);
...
TEST(Rank1Update, udu_downdate) // NOLINT
{
  // reverse test of the udu_update
...
{
  // clang-format off
  auto L = conversions::TriangularFromList<float64, 4, true, true>({
  ...
  const auto expL = conversions::TriangularFromList<float64, 4, true, true>({
...
{
  // reverse test of the ldl_update
