...
template class tracking::observation::RangeObservationModel<FullPolicy, StateDefCV>;
template class tracking::observation::RangeObservationModel<FactoredPolicy, StateDefCV>;

using RangeModel = tracking::observation::RangeObservationModel<FullPolicy, StateDefCV>;
...
TEST(RangeObservationModel, predictMeasurement__ReturnsRange) // NOLINT
{
  ...
  EXPECT_NEAR(predicted.at_unsafe(RangeModel::MEAS_RANGE), 5.0, 1e-12);
}
...
TEST(RangeObservationModel, computeJacobian__NoDivisionByZeroAtOrigin) // NOLINT
{
  ...
  typename RangeModel::JacobianMatrix H{};
  obs.computeJacobian(H, state, egoMotion);

  for (auto row = 0; row < RangeModel::DimZ; ++row)
  {
    for (auto col = 0; col < RangeModel::DimX; ++col)
    {
...
TEST(RangeObservationModel, predictMeasurement__AppliesSensorMountingPose) // NOLINT
{
  ...
  EXPECT_NEAR(predicted.at_unsafe(RangeModel::MEAS_RANGE), std::sqrt(20.0), 1e-9);
}
