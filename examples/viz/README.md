...
`examples/single_linear_object_tracking.cpp` and `examples/single_nonlinear_object_tracking.cpp`
optionally write a structured CSV (one row per simulation step) alongside their normal console
output. `render.py` reads that CSV and renders an animated GIF via a single generic renderer that
adapts its panels to the CSV columns present: a tracking-frame panel (ground truth, noisy
measurements, the filter's estimated track, and a 1-sigma covariance ellipse) and a position NEES
panel are always shown; the nonlinear example additionally gets a world-frame panel with the ego
vehicle's CTRV arc and the target's ground truth.
...
cmake .. && cmake --build . --target single_linear_object_tracking single_nonlinear_object_tracking

# Run an example, writing a CSV (path is the first CLI argument; defaults to
# single_linear_track.csv / single_nonlinear_track.csv in the current directory if omitted)
