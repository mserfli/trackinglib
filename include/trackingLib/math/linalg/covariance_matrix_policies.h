...
/// - A `value_type` type alias defining the floating-point type
...
///
/// This policy type creates standard full covariance matrixes that store the complete
...
///
/// \tparam ValueType_ Floating-point type for matrix elements (e.g., float32, float64)
///
/// Usage example:
/// \code
...
struct FullCovarianceMatrixPolicy: CovarianceMatrixPolicyBase
{
  ...
  /// \brief Template alias for instantiating covariance matrixes
  ...
  using Instantiate = CovarianceMatrixFull<value_type, Size>;
};

/// \brief Policy for using UDU factored covariance matrix representation
///
/// This policy type creates covariance matrixes using UDU factorization, which stores
...
struct FactoredCovarianceMatrixPolicy: CovarianceMatrixPolicyBase
{
  ...
  /// \return Concrete covariance matrix type for the specified size
  template <sint32 Size>
  using Instantiate = CovarianceMatrixFactored<value_type, Size>;
};
